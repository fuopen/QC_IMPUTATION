...
each_chr () {
	...
	chr_log="imputation/UBIOPREDAX_impute_chr${chr}_qctool_strict.log"
	#chr_snpstat="imputation/UBIOPREDAX_impute_chr${chr}.snpstat"
	qctool -g "$chr_impute" -s "$impute_sap" -maf 0.05 0.5 -snp-missing-call-rate 0.01  -snp-missing-rate 0.005 -hwe 4 -info 0.95 1 -og $chr_filter -omit-chromosome >$chr_log
}
